<!--card个人信息部分-->
<template>
  <div id="card">
		  <!--sessionstorage user信息里面图片名字是userFace-->
  		<img class="avatar" v-bind:src="user.userFace" v-bind:alt="user.name">
  		<p class="name">{{user.name}}</p>
  	</header>
  	<footer>
  		<input class="search" type="text" v-model="$store.state.filterKey" placeholder="search user...">
  	</footer>
  </div>
</template>

<script>
export default {
  name: 'card',
  data () {
    return {    
      	user: JSON.parse(window.sessionStorage.getItem('user'))  //当前登录用户，user的获取 
      
    }
  }
}
</script>

<style lang="scss" scoped>       //sass npm install sass-loader --save-dev/ npm install node-sass --save-dev
#card {
	padding: 12px;
  .avatar{
  	width: 40px;
  	height: 40px;
  	vertical-align: middle;/*这个是图片和文字居中对齐*/
  }
  .name {
  	display: inline-block;
  	padding: 10px;
  	margin-bottom: 15px;
  	font-size: 16px;
  }
  .search {
  	background: #26292E;
  	height: 30px;
  	line-height: 30px;
  	padding: 0 10px;
  	border: 1px solid #3a3a3a;
  	border-radius: 4px;
  	outline: none;/*鼠标点击后不会出现蓝色边框*/
    color: #FFF;
  }
}
</style>
